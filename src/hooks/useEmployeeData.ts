...
// Store enrolled employees - starts empty, populated through enrollment
const enrolledEmployees: Employee[] = [];
...
export const useEmployeeData = () => {
  const [employees, setEmployees] = useState<Employee[]>(enrolledEmployees);
  const [incidents, setIncidents] = useState<Incident[]>(mockIncidents);
  const [activityLogs, setActivityLogs] = useState<ActivityLog[]>([]);
  const [searchTerm, setSearchTerm] = useState('');

  // Function to add new enrolled employee
  const addEnrolledEmployee = (employee: Employee) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Only update if there are enrolled employees
      if (employees.length > 0) {
        ...
        // Add random activity logs only for enrolled employees
        if (Math.random() > 0.7) {
